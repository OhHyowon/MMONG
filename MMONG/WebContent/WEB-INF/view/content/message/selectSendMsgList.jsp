				...
				jQuery.ajaxSettings.traditional = true;
		 		$.ajax({
					...
					success:function(val){
						if(val=='1'){
							alert("삭제되었습니다"); 
							location.href="/MMONG/message/selectSendMsg.do"; //controller
						}
					}
				}); 
			}
...
<div align="center">
	<table class="table">
		...
		<tbody>
		<c:choose>
			<c:when test="${not empty requestScope.sendList }">
				<c:forEach items="${requestScope.sendList }" var="list">
					<tr>
...
###################################### --%>
						...
						<!-- 첫페이지로 이동 -->
<a href="/MMONG/message/selectSendMsg.do?page=1">첫페이지</a>
...
-->
		 <c:choose >
			...
			<a href="/MMONG/message/selectSendMsg.do?page="${requestScope.pageBean.beginPage-1 }>◀</a>
		 </c:when>
		 <c:otherwise>
			 ◀
		 </c:otherwise>
 	...
 	<c:forEach begin="${requestScope.pageBean.beginPage }" end="${requestScope.pageBean.endPage }" var="page">
		<c:choose>
			<c:when test="${requestScope.pageBean.page!=page }">
				<!-- 현재 페이지가 아니라면 -->
			<a href="/MMONG/message/selectSendMsg.do?page=${page}">${page }</a>
			</c:when>
			<c:otherwise>
				&nbsp;[${page }]&nbsp;
			</c:otherwise>
...
-->
	...
	<c:choose>
		<c:when test="${requestScope.pageBean.nextPageGroup }">
			<a href="/MMONG/message/selectSendMsg.do?page=${requestScope.pageBean.endPage+1}">▶</a>
		</c:when>
		<c:otherwise>
			▶
		</c:otherwise>
...
<a href="/MMONG/message/selectSendMsg.do?page=${requestScope.pageBean.totalPage}">마지막페이지</a>
