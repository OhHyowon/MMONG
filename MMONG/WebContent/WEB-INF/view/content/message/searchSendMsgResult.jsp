	...
	<table class="table">
		<thead>
			<tr>
				...
				<td style="width: 150px;padding:10px 0px 10px 0px;">받는이</td>
				<td style="width:150px;padding:10px 0px 10px 0px;">제목</td>
				<td style="width: 150px;padding:10px 0px 10px 0px;">내용</td>
				<td style="width: 100px;padding:10px 0px 10px 0px;">날짜</td>
				<td style="width: 100px;padding:10px 40px 10px 0px;">상태</td>
			</tr>
		</thead>
		<tbody>
		<c:choose>
				...
				<c:forEach items="${requestScope.searchSendList }" var="list">
					<tr>
						...
						<td style="width: 100px;padding:10px 40px 10px 0px;">
							<c:choose>
								<c:when test="${list.state eq 0 || list.state eq 2}">
									읽지 않음
								</c:when>
								<c:otherwise>
									읽음
...
###################################### --%>
						...
						<!-- 첫페이지로 이동 -->
<a href="/MMONG/message/searchSendMsg.do?page=1&searchOpt=${requestScope.search_opt}&search=${requestScope.search_txt}">첫페이지</a>
...
-->
		 <c:choose >
			...
			<a href="/MMONG/message/searchSendMsg.do?page=${requestScope.pageBean.beginPage-1 }&searchOpt=${requestScope.search_opt}&search=${requestScope.search_txt}">◀</a>
		 </c:when>
		 <c:otherwise>
			 ◀
		 </c:otherwise>
 	...
 	<c:forEach begin="${requestScope.pageBean.beginPage }" end="${requestScope.pageBean.endPage }" var="page">
		<c:choose>
			<c:when test="${requestScope.pageBean.page!=page }">
				<!-- 현재 페이지가 아니라면 -->
			<a href="/MMONG/message/searchSendMsg.do?page=${page}&searchOpt=${requestScope.search_opt}&search=${requestScope.search_txt}">${page }</a>
			</c:when>
			<c:otherwise>
				&nbsp;[${page }]&nbsp;
			</c:otherwise>
...
-->
	...
	<c:choose>
		<c:when test="${requestScope.pageBean.nextPageGroup }">
			<a href="/MMONG/message/searchSendMsg.do?page=${requestScope.pageBean.endPage+1}&searchOpt=${requestScope.search_opt}&search=${requestScope.search_txt}">▶</a>
		</c:when>
		<c:otherwise>
			▶
		</c:otherwise>
